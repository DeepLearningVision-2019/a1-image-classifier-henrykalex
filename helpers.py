...
def standardize_input(image, width, height):
    ...
    # cv2.resize
    standard_im = cv2.resize(image,(width, height), interpolation = cv2.INTER_CUBIC)
...
def standardize(image_list):
    ...
    for item in image_list:
        ...
        standar_image = standardize_input(item[0],1100, 600 )
        # Create a numerical label
        item_label = encode(item[1])
        # Append the image, and it's one hot encoded label to the full, processed list of image data
        standard_list.append((standar_image,item_label))
